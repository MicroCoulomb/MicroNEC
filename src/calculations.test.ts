...
describe('NEC quick-check calculations', () => {
  ...
  it('looks up single-phase motors and derives their OCPD poles', () => {
    ...
    expect(motorSizing({ ...base, voltage: 230 }).rows.find(([label]) => label === 'Motor load per phase')?.[1]).toBe('6,440 VA')
    const selectedTwoPole = motorSizing({ ...base, voltage: 115, poles: 2 })
    expect(selectedTwoPole.primary).toContain('2-pole')
    expect(selectedTwoPole.rows.find(([label]) => label === 'Motor load per phase')?.[1]).toBe('3,220 VA')
