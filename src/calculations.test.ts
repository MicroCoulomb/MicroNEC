...
describe('NEC quick-check calculations', () => {
  ...
  it('sizes the binder continuous-load example at 4/0 copper and 250 A', () => {
    ...
    expect(result.rows.find(([label]) => label === 'Load apparent power per phase')?.[1]).toBe('51 kVA')
  })
  ...
  it('uses MCA and MOCP for HVAC equipment', () => {
    ...
    expect(result.rows.find(([label]) => label === 'MCA apparent load per phase')?.[1]).toBe('7,205 VA')
  })
  ...
  it('looks up single-phase motors and derives their OCPD poles', () => {
    ...
    expect(motorSizing({ ...base, voltage: 115 }).rows.some(([label]) => label === 'Motor load per phase')).toBe(false)
    expect(motorSizing({ ...base, voltage: 230 }).rows.find(([label]) => label === 'Motor load per phase')?.[1]).toBe('3,220 VA')
    const selectedTwoPole = motorSizing({ ...base, voltage: 115, poles: 2 })
    expect(selectedTwoPole.primary).toContain('2-pole')
    expect(selectedTwoPole.rows.find(([label]) => label === 'Motor load per phase')?.[1]).toBe('1,610 VA')
  ...
  it('identifies three-phase HP lookups as Table 430.250', () => {
    ...
    expect(result.rows.find(([label]) => label === 'Motor load per phase')?.[1]).toBe('2,005 VA')
